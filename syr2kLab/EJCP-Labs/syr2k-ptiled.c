   ...
   int SI, int SK, int SJ)
{
  ...
  for (ti = 0; ti < N; ti+=SI) {
    for (tk = 0; tk < M; tk+=SK) {
      for (tj = 0; tj < N; tj+=SJ) {
          for(i = ti; (i < ti+SI) && (i < N); i++){
            for(k = tk; (k < tk+SK) && (k < M); k++){
                for(j = tj; (j < tj+SJ) && (j < N) ; j++){
...
int main(int argc, char** argv)
{
  ...
  if (argc < 6) {
    printf("usage ./syr2k N M SI SK SJ\n");
    return 0;
  }
  ...
  kernel_syr2k  (N, M, *((double(*)[N][N])C), *((double(*)[N][M])A), *((double(*)[N][M])B), SI, SJ, SK);
